...
class ArrayOfStatusCode extends AbstractStructArrayBase
{
     ...
     * @return \StructType\StatusCode[]|null
     */
    ...
    public static function validateStatusCodeForArrayConstraintsFromSetStatusCode(array $values = array())
    {
        ...
        foreach ($values as $arrayOfStatusCodeStatusCodeItem) {
            ...
            if (!$arrayOfStatusCodeStatusCodeItem instanceof \StructType\StatusCode) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The StatusCode property can only contain items of type \StructType\StatusCode, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \StructType\StatusCode[] $statusCode
     * @return \ArrayType\ArrayOfStatusCode
     */
     ...
     * @throws \InvalidArgumentException
     * @param \StructType\StatusCode $item
     * @return \ArrayType\ArrayOfStatusCode
     */
    public function addToStatusCode(\StructType\StatusCode $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\StatusCode) {
            throw new \InvalidArgumentException(sprintf('The StatusCode property can only contain items of type \StructType\StatusCode, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
